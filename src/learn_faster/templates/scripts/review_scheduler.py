    ...
    try:
        ...
        tell application "Reminders"
            tell list "Learn FASTER"
                make new reminder with properties {{name:"Review: {concept} ({topic_slug})", due date:date "{reminder_date}", body:"Time to review '{concept}' from your {topic_slug} learning. Run /review in OpenCode."}}
            end tell
        end tell
        '''
        ...
        try:
            ...
            """
            subprocess.run(
                ["osascript", "-e", create_list_script], check=True, capture_output=True
            )
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print("Usage:")
        print(
            "  Add concept:    python3 review_scheduler.py add <topic_slug> <concept>"
        )
        print(
            "  Mark reviewed:  python3 review_scheduler.py review <topic_slug> <concept>"
        )
